...
public class GraphControllerTests extends AbstractTest {
	...
	public void addGraph() throws Exception {
		...
		assertEquals(200, status);
		String content = mvcResult.getResponse().getContentAsString();
		Graph resultGraph = new ObjectMapper().readValue(content, Graph.class);
		assertEquals(graph.getGraphId(), resultGraph.getGraphId());
		assertEquals(graph.getDescription(), resultGraph.getDescription());
	}

	@Test
	public void deleteGraph() throws Exception {
		String uri = "/deleteGraph/2";
